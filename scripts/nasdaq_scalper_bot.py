#!/usr/bin/env python3
"""
Bot 5 — NASDAQ-100 (NQ) Scalper Bot
...
# ── TRADOVATE MEVCUT NQ KONTRAT ID ───────────────────────────────────
_nq_contract_id = None

def get_nq_contract_id():
    """Mevcut aktif NQ kontrat ID'sini bul."""
...
def get_candles_yfinance(tf_key, limit=100):
    """yfinance fallback — NQ=F sembolünden veri çek."""
    try:
        import yfinance as yf
        yf_interval_map = {
            "1m":  "1m",
...
def load_params():
    try:
        r = requests.get(f"{BASE_URL}/BotCache", headers=HEADERS(),
                         params={"key": "bot5_params"}, timeout=8)
        if r.status_code == 200:
            for item in r.json():
                if item.get("key") == "bot5_params":
    ...
    try:
        ...
        if existing:
            ...
            requests.post(f"{BASE_URL}/BotCache", headers=HEADERS(),
                          json={"key": "bot5_params", "value": val}, timeout=8)
    except Exception as e:
...
def get_dynamic_threshold(candles_15m, base_threshold=3.5):
    """
    NASDAQ 100 için ATR dinamik eşik.
    Normal ATR%: ~0.05-0.15 (NQ ~20000 seviyesinde)
    """
...
def analyze(params):
    ...
    if   weighted_avg >=  dyn_threshold: direction = "LONG"
        ...
        cache_key_active = f"bot5_alert_active_{alert_dir}"
        opposite         = "SHORT" if alert_dir == "LONG" else "LONG"
        set_cache(f"bot5_alert_active_{opposite}", "0")
        ...
        set_cache(f"bot5_alert_{alert_dir}", str(int(time.time())))
        price  = get_price()
        tf_str = " | ".join([f"{k}:{v:+d}" for k, v in scores.items()])
        send_telegram(
            f"⚠️ *NASDAQ 100 HAZIR OL — {alert_dir}*\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            ...
            f"━━━━━━━━━━━━━━━━━━\n"
            f"📡 *Bot 5 — NASDAQ 100 Scalper*"
        )
        print(f"  HAZIR OL: {alert_dir} skor:{weighted_avg:.2f}")
        return None
    else:
        set_cache("bot5_alert_active_LONG",  "0")
        set_cache("bot5_alert_active_SHORT", "0")
        return None
    ...
    if not vol_ok:
        ...
        # NASDAQ 100: piyasa saatleri dışında hacim çok düşük olabilir, yine de devam
...
def self_learn(params):
    try:
        r = requests.get(f"{BASE_URL}/ActiveTrade", headers=HEADERS(),
                         params={"symbol": "NQ"}, timeout=10)
        if r.status_code != 200:
            return params

        all_trades = [t for t in r.json()
                      if t.get("symbol") == "NQ" and t.get("status") != "OPEN"]
...
def get_open_trade():
    try:
        r = requests.get(f"{BASE_URL}/ActiveTrade", headers=HEADERS(),
                         params={"symbol": "NQ", "status": "OPEN"}, timeout=10)
        if r.status_code == 200:
...
def run_watchdog():
    ...
    if direction == "LONG" and price <= sl:
        if result_pct >= 0:
            label = "〽️ NASDAQ 100 BREAKEVEN ÇIKTI"
        elif abs(result_pct) < 0.05:
            label = "〽️ NASDAQ 100 BREAKEVEN ÇIKTI"
        else:
            label = "🛑 NASDAQ 100 SL ULAŞTI"
...
# ── SCAN ──────────────────────────────────────────────────────────────
def run_scan():
    print("🔍 Bot5 Scan başlıyor (NASDAQ 100 / NQ)...")
    params = load_params()
    ...
    # Self-learn
    last_learn = get_cache("bot5_last_learn")
    now_ts = int(time.time())
    if not last_learn or (now_ts - int(last_learn)) >= 900:
        params = self_learn(params)
        set_cache("bot5_last_learn", str(now_ts))
    ...
    send_telegram(
        f"🚨 *BOT 5 — NASDAQ 100 SİNYALİ*\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🪙 *NQ (NASDAQ-100)* | {dir_str}\n"
        f"📍 Giriş: `{price:.2f}`\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
